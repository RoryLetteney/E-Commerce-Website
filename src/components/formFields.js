...
export class LongGrayButton extends Component {
    render() {
        const { className, title, labelTitle, type, onClick, input } = this.props;
        return (
            <div className={`${className} form-button-long-gray`}>
                <label className="form-button-long-gray__label">{labelTitle}</label>
                <button className={`form-button-long-gray__button with-gray`} type={type} {...input} onClick={onClick}>{title}</button>
            </div>
        )
    }
}